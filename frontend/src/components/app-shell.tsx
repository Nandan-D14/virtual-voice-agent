...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen bg-[#030303] overflow-hidden text-white selection:bg-cyan-500/30">
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 bg-[#030303]/80 backdrop-blur-xl border-b border-white/5 z-50 flex items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-black tracking-tighter italic uppercase text-lg">Nexus</span>
        </Link>
        <button
      ...
      {/* Sidebar */}
      <motion.aside
        className={`fixed md:sticky top-0 left-0 z-40 h-screen w-64 bg-[#0a0a0a] border-r border-white/5 flex flex-col transition-transform duration-300 ease-in-out ${
      ...
      >
        ...
        <div className="px-4 py-2">
          <Link
            href="/"
            className="flex items-center gap-2 justify-center w-full px-4 py-3 bg-white text-black rounded-xl font-bold text-sm tracking-wider uppercase hover:bg-cyan-400 transition-colors shadow-lg active:scale-95"
          >
